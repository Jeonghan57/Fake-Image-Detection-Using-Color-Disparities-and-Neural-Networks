...
train_imgs = ImageFolder(f"./Datafolder/LSUN/train(church_StyleGAN2)/",
                         transform=transforms.Compose([
                                 transforms.ToTensor()]
))

test_imgs = ImageFolder(f"./Datafolder/LSUN/test(church_StyleGAN2)/",
                        transform=transforms.Compose([
                                 transforms.ToTensor()]
))
...
# 사전 학습이 끝난(Pre-trained)) 모델 불러오기 및 정의
...
net.classifier = nn.Sequential(*[net.classifier[i] for i in range(0)])
net.fc = nn.Linear(15, 2)
...
def train_net(net, train_loader, test_loader,
    ...
    torch.save(net.state_dict(), f"./result/LSUN/epoch(church(PGGAN)[retrain]_VGG19)_{n_iter-1}.pth")

def Evaluate_Networks(Net):
    save_path = "./result/LSUN/ver 1/"
    # data
    Net.load_state_dict(torch.load(save_path + "epoch(cat(PGGAN)_SqueezeNet1_1)_49.pth"), strict=False)
